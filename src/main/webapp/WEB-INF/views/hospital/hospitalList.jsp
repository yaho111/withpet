...
<section id="section_hospital_list" class="py-5">
    <div class="container">
        <h2 class="body-title">동물병원 게시판</h2>
            ...
            <c:if test="${not empty list}">
                ...
                <c:forEach var="hospital" items="${list}">
                    <tr>
                        ...
                        <td>
                            <a href="hospitalView?hos_no=${hospital.hos_no}&page=${paging.currentPage}">${hospital.hos_name}</a>
                        </td>
            ...
            <div class="search">
                <select name="search" class="form-select">
                    <option value="hos_name"
                            <c:if test="${search=='hos_name'}">selected="selected" </c:if>>병원 이름
                    </option>
                    <option value="hos_loc"
                            <c:if test="${search=='hos_loc'}">selected="selected" </c:if>>위치
                    </option>
                    <option value="hos_content"
                            <c:if test="${search=='hos_content'}">selected="selected" </c:if>>내용
                    </option>
                    <option value="hos_writer"
                            <c:if test="${search=='hos_writer'}">selected="selected" </c:if>>작성자
                    </option>
                </select>
        ...
        <nav>
            <ul class="pagination">
                ...
                <li><a class="page-link"
                       href="hospitalList?page=1&search=${search}&keyword=${keyword}&sortValue=${sortValue}"> << </a>
                </li>
                </c:if>
                ...
                <c:if test="${paging.startPage > paging.pagePerBlk}">
                    <li><a class="page-link"
                           href="hospitalList?page=${paging.startPage - 1}&search=${search}&keyword=${keyword}&sortValue=${sortValue}">
                        < </a></li>
                </c:if>
                ...
                <c:forEach var="i" begin="${paging.startPage}" end="${paging.endPage}">
                    <li <c:if test="${paging.currentPage!=i}">class="active"</c:if>><a
                            class="page-link"
                            href="hospitalList?page=${i}&search=${search}&keyword=${keyword}&sortValue=${sortValue}">${i}</a>
                    </li>
                </c:forEach>
                <!-- 다음 블럭으로 이동 -->
                <c:if test="${paging.endPage < paging.totalPage}">
                    <li><a class="page-link"
                           href="hospitalList?page=${paging.endPage + 1}&search=${search}&keyword=${keyword}&sortValue=${sortValue}">
                        > </a></li>
                </c:if>
                ...
                <c:if test="${paging.endPage < paging.totalPage}">
                    <li><a class="page-link"
                           href="hospitalList?page=${paging.totalPage}&search=${search}&keyword=${keyword}&sortValue=${sortValue}">
                        >> </a></li>
                </c:if>
                </c:if>
                ...
                <c:if test="${empty keyword}">
                    ...
                    <!-- 다음 블럭으로 이동 -->
                    <c:if test="${paging.endPage < paging.totalPage}">
                        <li><a class="page-link" href="hospitalList?page=${paging.endPage + 1}&sortValue=${sortValue}">
                            > </a>
                        </li>
                    </c:if>
                    ...
                    <!-- 마지막 페이지로 이동 -->
                    <c:if test="${paging.endPage < paging.totalPage}">
                        <li><a class="page-link" href="hospitalList?page=${paging.totalPage}&sortValue=${sortValue}">
                            >> </a>
                        </li>
                    </c:if>
                </c:if>
