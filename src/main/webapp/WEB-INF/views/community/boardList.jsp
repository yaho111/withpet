...
<section class="py-5">
	<div class="container">
		...
		<form action="boardList" align="center">
			...
			<select name="search" id="keyword_list" class="form-select" >
              ...
              <li><a class="page-link" href="boardList?pageNum=1&search=${search}&keyword=${keyword}"><<</a></li>
                </c:if> 
			   <c:if test="${paging.startPage > 10 }">								
	       <li> <a class="page-link" href="boardList?pageNum=${startPage - 1}&search=${search}&keyword=${keyword}"> < </a> </li>
							</c:if>
	...
	<li> <a class="page-link" href="boardList?pageNum=${endPage + 1}&search=${search}&keyword=${keyword}"> > </a> </li>
							</c:if> 
							<!-- 마지막 페이지 이동 --> 
            <c:if test="${paging.currentPage!=totalPage}">
                  <li><a class="page-link" href="boardList?pageNum=${paging.totalPage}&search=${search}&keyword=${keyword}">
                    << 
                  </a></li>
            </c:if>
			</c:if>
            ...
            <c:if test="${paging.currentPage!=1}">
                 <li><a class="page-link" href="boardList?pageNum=1"> << </a></li>
            </c:if>
				 			<c:if test="${startPage > 10 }">
			<li><a class="page-link" href="boardList?pageNum=${startPage - 1}"> < </a> </li>
							</c:if>
			           <c:forEach var="i" begin="${startPage}" end="${endPage}">
					<li <c:if test="${currentPage==i}">class="active"</c:if>>
					<a class="page-link" href="boardList?pageNum=${i}">${i}</a></li>
							...
							<c:if test="${endPage < totalPage}">
			<li><a class="page-link" href="boardList?pageNum=${endPage + 1}"> > </a> </li>
							</c:if>
							<!-- 마지막 페이지 이동 -->
      <c:if test="${paging.currentPage!=paging.totalPage}">
         <li><a class="page-link" href="boardList?pageNum=${paging.totalPage}"> >> </a></li>
      </c:if>
						</c:if>
					</li>
				</ul>
